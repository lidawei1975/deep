...
public:
     ...
     * 
     */
    static std::vector<double> ppm, intensity;
     ...
     * The second index is for the possible multiple matches of compounds to the peak.
     */
    static std::vector<std::vector<int>> matching_compound_index; //compound index. 
    static std::vector<double> clashing_additional_cost; //Addtional cost caused by peak matching clasher (multiple compounds match one peak)
...
private:
    ...
    /**
     * ppm per point in db, provided by user. default is 0.00018 (for Gissmo simulated spectra) if not.
    ...
    */
    ...
    double v_fitted2; //fitted value (concentration of matched compound) using peak height * experimental peak width (effective peak width, defined as volume/height)
    ...
    */
    ...
    bool fit_volume(bool b_keep_bdl=false,bool b_keep_covered=true);
...
public:
    ...
    db_match_1d(double,double,double,double); //cutoff, cutoff_pattern,water_width,step2
    ~db_match_1d();
    ...
    /**
     * load database peak list from a file, such as bmse000047_1.tab
     * cutoff_group is used to define peak groups.
    */
    bool load_db(double cutoff_group,double db_cutoff);
    ...
    */
    ...
    bool save_summary_json(Json::Value &root,Json::Value &root_name,Json::Value &root_link);
    bool save_summary(std::ofstream &);
    bool copy_and_ref_correction_spectrum(double ref_correction);
    bool clear_clashing_additional_cost();
