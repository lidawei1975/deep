 ...
 * fid_data_double (unused one will have size 0)
...
bool fid_1d::read_bruker_acqus_and_fid(const std::string &acqus_file_name, const std::vector<std::string> &fid_data_file_names)
{
    ...
    */
     ...
     * Here we leave them interleaved in fid_data_double or fid_data_int
     */
    
    fid_data_double.clear(); 
    fid_data_double.resize(ndata_bruker);
    ...
    for(int i=0;i<fid_data_file_names.size();i++)
    {
        ...
        if (data_type == FID_DATA_TYPE_INT32)
        {
            ...
            nread=fread(&fid_data_int[0], sizeof(int32_t), ndata_bruker, fp_fid_data);
            if(nread!=ndata_bruker)
            {
        ...
        else if (data_type == FID_DATA_TYPE_FLOAT64)
        {
            ...
            nread=fread(&temp_fid_data_double[0], sizeof(double), ndata_bruker, fp_fid_data);
            if(nread!=ndata_bruker)
            {
                std::cout << "Error: cannot read " << ndata_bruker << " float64 from file " <<  fid_data_file_names[i] << std::endl;
                return false;
            }
        }
        ...
        */
        if (data_type == FID_DATA_TYPE_INT32)
        {
            for (int i = 0; i < ndata_bruker; i++)
            {
                fid_data_double[i] += (double)fid_data_int[i];
            }
        }
        else
        {
            for (int i = 0; i < ndata_bruker; i++)
            {
                fid_data_double[i] += temp_fid_data_double[i];
            }
        }
        fclose(fp_fid_data);
    }
 ...
 */
bool fid_1d::read_jcamp(std::string file_name, std::map<std::string, std::string> &udict) const
{
 ...
 */
bool fid_1d::read_jcmap_line(std::ifstream &infile, std::string line, std::string &key, std::string &value) const
{
    /**
...
bool fid_1d::write_nmrpipe_fid(const std::string outfname) const
{
    ...
    */
    ...
    /**
     * convert fid_data_double to temp_data_float, real part only: 0,2,4,6,...
     */
    std::vector<float> temp_data_float;
    temp_data_float.resize(ndata_bruker);
    for (int i = 0; i < ndata; i++)
    {
        temp_data_float[i] = (float)fid_data_double[i*2];
    }
    fwrite(temp_data_float.data(), sizeof(float), ndata_bruker, fp);

    /**
     * convert fid_data_double to temp_data_float, imaginary part only: 1,3,5,7,...
     */
    for (int i = 0; i < ndata; i++)
    {
        temp_data_float[i] = (float)fid_data_double[i*2+1];
    }
    fwrite(temp_data_float.data(), sizeof(float), ndata_bruker, fp);
...
/**
 * @brief fid_1d::run_fft_and_rm_bruker_filter: run fft on fid_data_double
...
bool fid_1d::run_fft_and_rm_bruker_filter()
{
    ...
    */
    ...
    for (int i = 0; i < ndata; i++)
    {
        in[i].r = fid_data_double[i * 2];
        in[i].i = fid_data_double[i * 2 + 1];
    }
    ...
    */
    ...
    {
        spectrum_real[i] = out[i].r/sqrt(float(ndata_frq));
        spectrum_imag[i] = out[i].i/sqrt(float(ndata_frq)); // scale by sqrt(ndata_frq) to be consistent with standard fft
    }

    /**
     * Important: this step need to be done before fft result swap and flip
    */
    remove_bruker_digitizer_filter(); 
    ...
    for (int i = 0; i < ndata_frq; i++)
    {
        spectrum_real_reoganized[ndata_frq - i] = (float)spectrum_real[(i + ndata_frq / 2) % ndata_frq]*receiver_gain;
        spectrum_imag_reoganized[ndata_frq - i] = (float)spectrum_imag[(i + ndata_frq / 2) % ndata_frq]*receiver_gain;
    }

    spectrum_real=spectrum_real_reoganized;
    spectrum_imag=spectrum_imag_reoganized;

    return true;
}

/**
 * @brief fid_1d::run_apodization: run apodization on fid_data_double
 * For Kaiser 2 pi:
 * sin_shift = 0.5 (p1)
 * end_point = 0.896 (p2)
 * power = 3.684 (p3)
 * 
*/
bool fid_1d::run_apodization(FID_APODIZATION_TYPE apodization_type, double p1,double p2,double p3,double p4,double p5,double p6)
{
    if(apodization_type==FID_APODIZATION_KAISER)
    {
        for (int i = 0; i < ndata; i++)
        {
            double sp = pow(sin(M_PI * p1 + M_PI * p2 / 2.0 / ndata * i),p3);
            if (i == 0)
            {
                sp = 0.5; //special case for first point
            }
            /** 
             * Now apply sp to both real and imaginary parts of fid
            */
            fid_data_double[i * 2] *= sp;
            fid_data_double[i * 2 + 1] *= sp;
        }
    }
    else
    {
        std::cout << "Error: apodization type " << apodization_type << " is not supported" << std::endl;
        return false;
    }
...
/**
 * @brief fid_1d::remove_bruker_digitizer_filter: remove Bruker digitizer filter
...
*/
bool fid_1d::remove_bruker_digitizer_filter()
{
    /** 
    ...
    */
     ...
     * spectrum_real and spectrum_imag
    */
    for(int i=0;i<ndata_frq;i++)
    {
        float phase=2*M_PI*grpdly*i/ndata_frq;
        float cos_phase=cos(phase);
        float sin_phase=sin(phase);

        float real=spectrum_real[i];
        float imag=spectrum_imag[i];

        spectrum_real[i]=real*cos_phase-imag*sin_phase;
        spectrum_imag[i]=real*sin_phase+imag*cos_phase;
    }
