...
*/
//peak rel
std::vector<double> match_1d_static_varibles::ppm, match_1d_static_varibles::intensity;
...
std::vector<double> match_1d_static_varibles::clashing_additional_cost;
...
const double match_1d_static_varibles::cost_overall_shift=1;
...
int main(int argc, char **argv)
{
    ...
    args2.push_back("no");
    args3.push_back("Print result to stdout in JSON format while suppressing other output (no)");
    ...
    */
    ...
    args.push_back("-db-id");
    args2.push_back("colmar0005_1");
    args3.push_back("database item ID, none means read from -db");

    args.push_back("-lb");
    args2.push_back("0.0");
    args3.push_back("lower bound of peak height, matched by the highest peak in the DB (0.0)");

    args.push_back("-ub");
    args2.push_back("1.0");
    args3.push_back("upper bound of peak height, matched by the highest peak in the DB (1.0)");
    ...
    args.push_back("-simulate_database");
    args2.push_back("yes");
    args3.push_back("Simulate database peaks from spin simulation on the fly (yes)");
    ...
    args2.push_back("0.02");
    args3.push_back("ppm cutoff to define peak groups. Not applicable if simulate_database is yes");

    args.push_back("-step");
    args2.push_back("0.00018");
    args3.push_back("ppm per point in database peak list. Not applicable if simulate_database is yes");
    ...
    bool b_stdout = cmdline.query("-stdout") == "yes" || cmdline.query("-stdout") == "y" || cmdline.query("-stdout") == "1";

    if (b_stdout)
    {
        match_1d_static_varibles::n_verbose_match_1d = -1;
    }
    else
    {
    ...
    double cutoff_group = std::stod(cmdline.query("-group"));
    double step = atof(cmdline.query("-step").c_str());
    ...
    bool b_simulate_database;
    std::string simulate_database = cmdline.query("-simulate_database");
    b_simulate_database = simulate_database.at(0) == 'y' || simulate_database.at(0) == 'Y' || simulate_database.at(0) == '1';
    ...
    {
        db_match_1d d(cutoff_h, cutoff_h2,water_width,step);
    ...
    std::string db_id = cmdline.query("-db-id");
    if(db_id == "none")
    {
        /**
         * Read from -db
    ...
    else
    {
        ...
        lbs.push_back(atof(cmdline.query("-lb").c_str()));
        ubs.push_back(atof(cmdline.query("-ub").c_str()));
    }
    ...
    */
    ...
    for(int m=0;m<db_ids.size();m++)
    {
        ...
        */
        ...
        {
            //remove .tab from db_file_id if it exists (at the end)
            if(db_id.find(".tab")!=std::string::npos)
            {
                db_id=db_id.substr(0,db_id.find(".tab"));
            }
            /**
    ...
    */
    ...
    for (int i0 = 0; i0 < ndx_db.size(); i0++) 
    {
         ...
         */
            ...
            db_match_list[i].clear_clashing_additional_cost();
        }
        ...
        */
        ...
        {
            b_load=db_match_list[i].load_db(cutoff_group, db_height_cutoff);
        }
    ...
    if(b_stdout)
    {
        ...
        for (int i0 = 0; i0 < ndx_db.size(); i0++) 
        {
            ...
            int i=ndx_db[i0]; // i is the index of db_match_list
            db_match_list[i].save_summary_json(root,root["name"],root["link"]);
        }
        root["webapp"]["median_database_peak_width"] = median_database_peak_width;
        root["webapp"]["median_experimental_peak_width"] = median_experimental_peak_width;
        root["webapp"]["width-factor"] = std::stod(cmdline.query("-width-factor"));
        root["webapp"]["cutoff_h"] = cutoff_h;
        root["webapp"]["cutoff_h2"] = cutoff_h2;
        root["webapp"]["ref"] = ref;
        root["webapp"]["water_width"] = water_width;
    ...
    else
    {
         ...
         * The user can specify multiple output files. The output files can be .txt or .json files.
         * Only first one of each type will be used. Files with other types will be ignored.
         */
        
        std::ofstream fout;               // output file stream for .txt file
        std::ofstream fout_json;          // output file stream for .json file
        std::ofstream fout_json_addinfor; // output filename for additional information. This file is used to store the additional information for each compound in the .json file
        Json::Value root;                 // define a json object as root
        Json::Value root2;                // define a json object as root for additional information
        ...
        for (int j = 0; j < outfnames.size(); j++)
        {
            ...
            // if outfnames[j] ends with .json, write to it
            if ((!fout_json.is_open()) && outfnames[j].substr(outfnames[j].size() - 5, 5) == ".json")
            {
                fout_json.open(outfnames[j]);
                std::string addinfor_filename = outfnames[j].substr(0, outfnames[j].size() - 5) + "_addinfor.json";
                fout_json_addinfor.open(addinfor_filename);
            }
        }
        ...
        for (int i0 = 0; i0 < ndx_db.size(); i0++) 
        {
            ...
            int i=ndx_db[i0]; // i is the index of db_match_list
            // write to fout if fout is open
            if (fout.is_open())
            {
                db_match_list[i].save_summary(fout);
                fout<<std::endl;
            }

            if (fout_json.is_open())
            {
                if (db_match_list[i].save_summary_json(root[counter],root2["name"][counter],root2["link"][counter]))
                {
                    counter++;
                }
            }
        ...
        // close fout, fout_json and fout_json_addinfor
        // write json to fout_json and fout_json_addinfor before closing them
        if (fout.is_open())
        {
            fout.close();
        }
        if (fout_json.is_open())
        {
            ...
            // add user input to this json file too
            root2["user_input"]["cutoff_h"] = cutoff_h;
            root2["user_input"]["cutoff_h2"] = cutoff_h2;
            root2["user_input"]["ref"] = ref;
            root2["user_input"]["water_width"] = water_width;
            root2["user_input"]["folder"] = folder; //500,850, 80, etc
            //exp and db peak width median, required by the web app
            root2["webapp"]["median_database_peak_width"] = median_database_peak_width;
            root2["webapp"]["median_experimental_peak_width"] = median_experimental_peak_width;

            fout_json_addinfor << root2;
            fout_json_addinfor.close();
        }
    }
