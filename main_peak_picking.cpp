...
int main(int argc, char **argv)
{
    ...
    args3.push_back("Output file name in nmrDraw format. Will also write another file in Sparky format with .list extension. (peaks.tab)");
    ...
    if (cmdline.query("-h") != "yes")
    {
        ...
        if(x.init(infname)) //read and zero filling
        {
            ...
            x.ann_peak_picking(); //picking and fitting, output
            x.output_picking(outfname);
        }
